...
where job_id in ('IT_PROG', 'ST_MAN') and(
	...
	salary = (select min(salary) from employees where job_id in ('IT_PROG', 'ST_MAN')));
...
where cntr.country_name = 'United Kingdom';
...
join countries as cntr
on loc.country_id = cntr.country_id
where cntr.country_name = 'United States of America' and 
emp.salary > (select avg(emp.salary)
		from employees as emp
		join departments as dep
		on emp.department_id = dep.department_id 
